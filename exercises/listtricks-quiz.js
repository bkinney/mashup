  ...
  $.fn.listtricksQuiz = function(options) {
	...
	var plugin = this;
	 ...
	 return $(this).each(function(){//divs are sent here
		...
		$(this).addClass("quiz").prepend('<div class="qscore"/>').append('<div class="feedback">Click letter buttons to reveal answer options. Click again to select.</div>');
 	...
 	$.fn.showQuestion=function(options,context){
		...
		$(this).show().siblings().hide();
		 ...
		 $("#buttons > ol > li").each(function(i){
			...
			$(this).html('<button>' + String.fromCharCode(i + 97) + '</button>').click(function(obj){
				if(!$(obj.target).hasClass("active")){//touch
					...
					context.children("div.feedback").html("Click again to select this answer");
					return;
				}else{
